...
def main(fastq_path, json_stats_path):
    if fastq_path == "-":
        fastq_fhand = sys.stdin.buffer
    else:
        fpath = Path(fastq_path)
        if not fpath.exists():
    ...
    json_stats_path
    with open(json_stats_path, "w") as out_stats_fhand:
        calc_fastq_stats(fastq_fhand, out_stats_fhand)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: calc_fastq_stats <fastq_file>(- for stdin) <stats_json_file>")
        sys.exit(1)
    fastq_path = Path(sys.argv[1])
    json_stats_path = Path(sys.argv[2])
    main(fastq_path, json_stats_path)
